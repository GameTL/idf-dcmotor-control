...
static void update_arr(int m_index, float vel, float pos){
    ...
    if (flag == 4)
    {
        ...
        uart_write_bytes(UART_NUM_1, tx_line, len);
        flag = 0;
    }
            ...
            switch (event.type) {
                case UART_DATA:
                    ...
                    if (len > 0) {
                        ...
                        int parsed = sscanf((char *)data, "%f %f %f %f", &t_vel1, &t_vel2, &t_vel3, &t_vel4);
                        if (parsed == 4)
                        {
                            rx_target_rads_arr[0] = t_vel1;
                            rx_target_rads_arr[1] = t_vel2;
                            rx_target_rads_arr[3] = t_vel3;
                            rx_target_rads_arr[4] = t_vel4;
                        }
                        printf("Received data - Target Rads: %.2f %.2f %.2f %.2f\n", rx_target_rads_arr[0], rx_target_rads_arr[1], rx_target_rads_arr[2], rx_target_rads_arr[3]);
                        /*==============================================================================================================================================*/                                                                  
                    }
                    break;
...
void app_main(void)
{
    ...
    {   
        ...
        //     // printf("Writing to UART\n");                                                             
        //     uart_write_bytes(UART_NUM_1, rx_line, len);
        // }
        vTaskDelay(500);
    }
